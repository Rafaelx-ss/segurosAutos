...
use app\models\ReportescamposSearch;
...
/**
 * ReportescamposController implements the CRUD actions for Reportescampos model.
 */
class ReportescamposController extends Controller
{
    /**
     * {@inheritdoc}
     */
	...
	public function actionXportpdf($f){
		...
		$searchModel = new ReportescamposSearch();
		
		if(isset(Yii::$app->session['ReportescamposSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', '', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Reportescampos'")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Reportescampos'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['ReportescamposSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['ReportescamposSearch'])) {
			...
			if(isset($catalogo['catalogoID'])){
				...
				$objPHPExcel->getActiveSheet()->setTitle('Reportescampos_'.date('Ymdhis'));
				$objPHPExcel->setActiveSheetIndex(0);
				...
				header('Content-Disposition: attachment;filename="Reportescampos_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Reportescampos models.
     * @return mixed
     */
    public function actionIndex($f, $token){
        $searchModel = new ReportescamposSearch();
      

		if(isset($_GET['clear'])){
			Yii::$app->session['ReportescamposSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['ReportescamposSearch'])) {
					$params = Yii::$app->session['ReportescamposSearch'];
				}else{
					Yii::$app->session['ReportescamposSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['ReportescamposSearch'])){
					Yii::$app->session['ReportescamposSearch'] = $params;
				}else{
					$params = Yii::$app->session['ReportescamposSearch'];
				}		
    ...
    /**
     * Displays a single Reportescampos model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new Reportescampos model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate($f, $token)
    {
        $model = new Reportescampos();
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->reporteCampoID]);
			}
		}
    ...
    /**
     * Updates an existing Reportescampos model.
     * If update is successful, the browser will be redirected to the 'view' page.
     * @param integer $id
     * @return mixed
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionUpdate($f, $token, $id)
    {
        $model = $this->findModel($id);
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				return $this->redirect(['update', 'f'=>$f, 'token'=>$token, 'id' => $model->reporteCampoID, 'update'=>'true']);
			}
		}
    ...
    /**
     * Deletes an existing Reportescampos model.
    ...
    /**
     * Finds the Reportescampos model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Reportescampos the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Reportescampos::findOne($id)) !== null) {
            return $model;
        }
