...
use app\models\MensajesbitacoraidiomasSearch;
...
/**
 * MensajesbitacoraidiomasController implements the CRUD actions for Mensajesbitacoraidiomas model.
 */
class MensajesbitacoraidiomasController extends Controller
{
    /**
     * {@inheritdoc}
     */
	...
	public function actionXportpdf($f){
		...
		$searchModel = new MensajesbitacoraidiomasSearch();
		
		if(isset(Yii::$app->session['MensajesbitacoraidiomasSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'MensajesBitacoraIdiomas', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Mensajesbitacoraidiomas'")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Mensajesbitacoraidiomas'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['MensajesbitacoraidiomasSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['MensajesbitacoraidiomasSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'MensajesBitacoraIdiomas', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Mensajesbitacoraidiomas_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Mensajesbitacoraidiomas models.
     * @return mixed
     */
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['MensajesbitacoraidiomasSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['MensajesbitacoraidiomasSearch'])) {
					$params = Yii::$app->session['MensajesbitacoraidiomasSearch'];
				}else{
					Yii::$app->session['MensajesbitacoraidiomasSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['MensajesbitacoraidiomasSearch'])){
					Yii::$app->session['MensajesbitacoraidiomasSearch'] = $params;
				}else{
					$params = Yii::$app->session['MensajesbitacoraidiomasSearch'];
				}		
    ...
    /**
     * Displays a single Mensajesbitacoraidiomas model.
     * @param integer $id
     * @return mixed
    ...
    {
        $model = new Mensajesbitacoraidiomas();
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'MensajesBitacoraIdiomas', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="77"')->queryAll();
    ...
    /**
     * Creates a new Mensajesbitacoraidiomas model.
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'MensajesBitacoraIdiomas', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="77"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->mensajeBitacoraIdiomasID]);
			}
		}
    ...
    /**
     * Updates an existing Mensajesbitacoraidiomas model.
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'MensajesBitacoraIdiomas', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="77"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'id' => $model->mensajeBitacoraIdiomasID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'MensajesBitacoraIdiomas', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="77" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Mensajesbitacoraidiomas model.
    ...
    /**
     * Finds the Mensajesbitacoraidiomas model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Mensajesbitacoraidiomas the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Mensajesbitacoraidiomas::findOne($id)) !== null) {
            return $model;
        }
