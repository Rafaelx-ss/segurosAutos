...
use app\models\VersionesSearch;
...
/**
 * VersionesController implements the CRUD actions for Versiones model.
 */
class VersionesController extends Controller
{
	...
	public function actionXportpdf($f){
		...
		$searchModel = new VersionesSearch();
		
		if(isset(Yii::$app->session['VersionesSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'Versiones', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Versiones'")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Versiones'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['VersionesSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['VersionesSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'Versiones', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Versiones_'.date('ymdHis').'.xlsx"');
    ...
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['VersionesSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['VersionesSearch'])) {
					$params = Yii::$app->session['VersionesSearch'];
				}else{
					Yii::$app->session['VersionesSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['VersionesSearch'])){
					Yii::$app->session['VersionesSearch'] = $params;
				}else{
					$params = Yii::$app->session['VersionesSearch'];
				}		
    ...
    /**
     * Displays a single Versiones model.
     * @param integer $id
     * @return mixed
	...
	public function actionCreateform()
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'Versiones', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="33"')->queryAll();
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'Versiones', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="33"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->versionID]);
			}
		}
    ...
    /**
     * Updates an existing Versiones model.
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'Versiones', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="33"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'id' => $model->versionID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'Versiones', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="33" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Versiones model.
    ...
    /**
     * Finds the Versiones model based on its primary key value.
    ...
    {
        if (($model = Versiones::findOne($id)) !== null) {
            return $model;
        }
