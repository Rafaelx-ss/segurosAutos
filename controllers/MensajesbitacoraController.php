...
use app\models\MensajesbitacoraSearch;
...
/**
 * MensajesbitacoraController implements the CRUD actions for Mensajesbitacora model.
 */
class MensajesbitacoraController extends Controller
{
    /**
     * {@inheritdoc}
     */
	...
	public function actionXportpdf($f){
		...
		$searchModel = new MensajesbitacoraSearch();
		
		if(isset(Yii::$app->session['MensajesbitacoraSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'MensajesBitacora', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Mensajesbitacora'")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Mensajesbitacora'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['MensajesbitacoraSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['MensajesbitacoraSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'MensajesBitacora', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Mensajesbitacora_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Mensajesbitacora models.
     * @return mixed
     */
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['MensajesbitacoraSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['MensajesbitacoraSearch'])) {
					$params = Yii::$app->session['MensajesbitacoraSearch'];
				}else{
					Yii::$app->session['MensajesbitacoraSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['MensajesbitacoraSearch'])){
					Yii::$app->session['MensajesbitacoraSearch'] = $params;
				}else{
					$params = Yii::$app->session['MensajesbitacoraSearch'];
				}		
    ...
    /**
     * Displays a single Mensajesbitacora model.
     * @param integer $id
     * @return mixed
	...
	public function actionCreateform()
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'MensajesBitacora', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="76"')->queryAll();
    ...
    /**
     * Creates a new Mensajesbitacora model.
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'MensajesBitacora', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="76"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->mensajeBitacoraID]);
			}
		}
    ...
    /**
     * Updates an existing Mensajesbitacora model.
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'MensajesBitacora', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="76"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'id' => $model->mensajeBitacoraID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'MensajesBitacora', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="76" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Mensajesbitacora model.
    ...
    /**
     * Finds the Mensajesbitacora model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Mensajesbitacora the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Mensajesbitacora::findOne($id)) !== null) {
            return $model;
        }
