...
use app\models\Logaccesosapi;
use app\models\LogaccesosapiSearch;
...
/**
 * LogaccesosapiController implements the CRUD actions for Logaccesosapi model.
 */
class LogaccesosapiController extends Controller
{
    /**
     * {@inheritdoc}
     */
	...
	public function actionXportpdf($f){
		...
		$searchModel = new LogaccesosapiSearch();
		
		if(isset(Yii::$app->session['LogaccesosapiSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'LogAccesosAPI', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Logaccesosapi'")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Logaccesosapi'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['LogaccesosapiSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['LogaccesosapiSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'LogAccesosAPI', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				$objPHPExcel->getActiveSheet()->setTitle('Logaccesosapi_'.date('Ymdhis'));
				$objPHPExcel->setActiveSheetIndex(0);
				...
				header('Content-Disposition: attachment;filename="Logaccesosapi_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Logaccesosapi models.
     * @return mixed
     */
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['LogaccesosapiSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['LogaccesosapiSearch'])) {
					$params = Yii::$app->session['LogaccesosapiSearch'];
				}else{
					Yii::$app->session['LogaccesosapiSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['LogaccesosapiSearch'])){
					Yii::$app->session['LogaccesosapiSearch'] = $params;
				}else{
					$params = Yii::$app->session['LogaccesosapiSearch'];
				}		
    ...
    /**
     * Displays a single Logaccesosapi model.
     * @param integer $logAccesoApiId
	...
	public function actionCreateform()
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'LogAccesosAPI', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="43"')->queryAll();
    ...
    /**
     * Creates a new Logaccesosapi model.
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'LogAccesosAPI', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="43"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->logAccesoApiId]);
			}
		}
    ...
    /**
     * Updates an existing Logaccesosapi model.
     ...
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionUpdate($f, $logAccesoApiId, $establecimientoID)
    {
        $model = $this->findModel($logAccesoApiId, $establecimientoID);
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'LogAccesosAPI', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="43"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'logAccesoApiId' => $model->logAccesoApiId, 'establecimientoID' => $model->establecimientoID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $logAccesoApiId, $establecimientoID)
    {
		$fID = Yii::$app->globals->getFormulario($f);
        $model = $this->findModel($logAccesoApiId, $establecimientoID);
		$version = $model->versionRegistro;	
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'LogAccesosAPI', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="43" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Logaccesosapi model.
    ...
    /**
     * Finds the Logaccesosapi model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $logAccesoApiId
     * @param integer $establecimientoID
     * @return Logaccesosapi the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($logAccesoApiId, $establecimientoID)
    {
        if (($model = Logaccesosapi::findOne(['logAccesoApiId' => $logAccesoApiId, 'establecimientoID' => $establecimientoID])) !== null) {
            return $model;
        }
