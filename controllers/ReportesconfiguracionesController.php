...
use app\models\ReportesconfiguracionesSearch;
...
/**
 * ReportesconfiguracionesController implements the CRUD actions for Reportesconfiguraciones model.
 */
class ReportesconfiguracionesController extends Controller
{
    /**
     * {@inheritdoc}
     */
	...
	public function actionXportpdf($f){
		...
		$searchModel = new ReportesconfiguracionesSearch();
		
		if(isset(Yii::$app->session['ReportesconfiguracionesSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'ReportesConfiguraciones', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Reportesconfiguraciones'")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Reportesconfiguraciones'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['ReportesconfiguracionesSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['ReportesconfiguracionesSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'ReportesConfiguraciones', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Reportesconfiguraciones_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Reportesconfiguraciones models.
     * @return mixed
     */
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['ReportesconfiguracionesSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['ReportesconfiguracionesSearch'])) {
					$params = Yii::$app->session['ReportesconfiguracionesSearch'];
				}else{
					Yii::$app->session['ReportesconfiguracionesSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['ReportesconfiguracionesSearch'])){
					Yii::$app->session['ReportesconfiguracionesSearch'] = $params;
				}else{
					$params = Yii::$app->session['ReportesconfiguracionesSearch'];
				}		
    ...
    /**
     * Displays a single Reportesconfiguraciones model.
     * @param integer $id
     * @return mixed
    ...
    {
        $model = new Reportesconfiguraciones();
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'ReportesConfiguraciones', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="52"')->queryAll();
    ...
    /**
     * Creates a new Reportesconfiguraciones model.
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'ReportesConfiguraciones', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="52"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
    ...
    /**
     * Updates an existing Reportesconfiguraciones model.
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'ReportesConfiguraciones', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="52"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'ReportesConfiguraciones', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="52" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Reportesconfiguraciones model.
    ...
    /**
     * Finds the Reportesconfiguraciones model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Reportesconfiguraciones the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Reportesconfiguraciones::findOne($id)) !== null) {
            return $model;
        }
