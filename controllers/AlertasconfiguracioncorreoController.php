...
use app\models\Alertasconfiguracioncorreo;
use app\models\AlertasconfiguracioncorreoSearch;
...
/**
 * AlertasconfiguracioncorreoController implements the CRUD actions for Alertasconfiguracioncorreo model.
 */
class AlertasconfiguracioncorreoController extends Controller
{
    /**
     * {@inheritdoc}
     */
	...
	public function actionXportpdf($f){
		...
		$searchModel = new AlertasconfiguracioncorreoSearch();
		
		if(isset(Yii::$app->session['AlertasconfiguracioncorreoSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'AlertasConfiguracionCorreo', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Alertasconfiguracioncorreo' and regEstado=1 and activoCatalogo=1")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Alertasconfiguracioncorreo'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['AlertasconfiguracioncorreoSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['AlertasconfiguracioncorreoSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'AlertasConfiguracionCorreo', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Alertasconfiguracioncorreo_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Alertasconfiguracioncorreo models.
     * @return mixed
     */
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['AlertasconfiguracioncorreoSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['AlertasconfiguracioncorreoSearch'])) {
					$params = Yii::$app->session['AlertasconfiguracioncorreoSearch'];
				}else{
					Yii::$app->session['AlertasconfiguracioncorreoSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['AlertasconfiguracioncorreoSearch'])){
					Yii::$app->session['AlertasconfiguracioncorreoSearch'] = $params;
				}else{
					$params = Yii::$app->session['AlertasconfiguracioncorreoSearch'];
				}		
    ...
    /**
     * Displays a single Alertasconfiguracioncorreo model.
     * @param integer $id
     * @return mixed
    ...
    {
        $model = new Alertasconfiguracioncorreo();
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'AlertasConfiguracionCorreo', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="117"')->queryAll();
    ...
    /**
     * Creates a new Alertasconfiguracioncorreo model.
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'AlertasConfiguracionCorreo', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="117"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->alertaConfiguracionCorreoID]);
			}
		}
    ...
    /**
     * Updates an existing Alertasconfiguracioncorreo model.
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'AlertasConfiguracionCorreo', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="117"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'id' => $model->alertaConfiguracionCorreoID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'AlertasConfiguracionCorreo', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="117" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Alertasconfiguracioncorreo model.
    ...
    /**
     * Finds the Alertasconfiguracioncorreo model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Alertasconfiguracioncorreo the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Alertasconfiguracioncorreo::findOne($id)) !== null) {
            return $model;
        }
