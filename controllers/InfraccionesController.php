...
use app\models\InfraccionesSearch;
...
class InfraccionesController extends Controller
{
	...
	public function actionXportpdf($f){
		...
		if(isset(Yii::$app->session['InfraccionesSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'Infracciones', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Infracciones' and regEstado=1 and activoCatalogo=1")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Infracciones'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['InfraccionesSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['InfraccionesSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'Infracciones', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Infracciones_'.date('ymdHis').'.xlsx"');
    ...
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['InfraccionesSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['InfraccionesSearch'])) {
					$params = Yii::$app->session['InfraccionesSearch'];
				}else{
					Yii::$app->session['InfraccionesSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['InfraccionesSearch'])){
					Yii::$app->session['InfraccionesSearch'] = $params;
				}else{
					$params = Yii::$app->session['InfraccionesSearch'];
				}		
	...
	public function actionCreateform()
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'Infracciones', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="127"')->queryAll();
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'Infracciones', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="127"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->infraccionID]);
			}
		}
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'Infracciones', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="127"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'id' => $model->infraccionID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'Infracciones', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="127" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
