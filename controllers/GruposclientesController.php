...
use app\models\GruposclientesSearch;
...
/**
 * GruposclientesController implements the CRUD actions for Gruposclientes model.
 */
class GruposclientesController extends Controller
{
    /**
     * {@inheritdoc}
     */
	...
	public function actionXportpdf($f){
		...
		$searchModel = new GruposclientesSearch();
		
		if(isset(Yii::$app->session['GruposclientesSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'GruposClientes', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Gruposclientes' and regEstado=1 and activoCatalogo=1")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Gruposclientes'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['GruposclientesSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['GruposclientesSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'GruposClientes', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Gruposclientes_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Gruposclientes models.
     * @return mixed
     */
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['GruposclientesSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['GruposclientesSearch'])) {
					$params = Yii::$app->session['GruposclientesSearch'];
				}else{
					Yii::$app->session['GruposclientesSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['GruposclientesSearch'])){
					Yii::$app->session['GruposclientesSearch'] = $params;
				}else{
					$params = Yii::$app->session['GruposclientesSearch'];
				}		
    ...
    /**
     * Displays a single Gruposclientes model.
     * @param integer $id
     * @return mixed
	...
	public function actionCreateform()
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'GruposClientes', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="121"')->queryAll();
    ...
    /**
     * Creates a new Gruposclientes model.
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'GruposClientes', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="121"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->grupoClienteID]);
			}
		}
    ...
    /**
     * Updates an existing Gruposclientes model.
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'GruposClientes', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="121"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'id' => $model->grupoClienteID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'GruposClientes', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="121" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Gruposclientes model.
    ...
    /**
     * Finds the Gruposclientes model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Gruposclientes the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Gruposclientes::findOne($id)) !== null) {
            return $model;
        }
