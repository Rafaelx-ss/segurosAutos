...
use app\models\DireccionesclientesSearch;
...
/**
 * DireccionesclientesController implements the CRUD actions for Direccionesclientes model.
 */
class DireccionesclientesController extends Controller
{
	...
	public function actionXportpdf($f){
		...
		$searchModel = new DireccionesclientesSearch();
		
		if(isset(Yii::$app->session['DireccionesclientesSearch'])) {

			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(7, 'PDF', 'DireccionesClientes', $fID);
			
			$catalogo = Yii::$app->db->createCommand("SELECT catalogoID FROM Catalogos where nombreModelo='Direccionesclientes' and regEstado=1 and activoCatalogo=1")->queryOne();
			if(isset($catalogo['catalogoID'])){
				...
				if(isset($cabecera['cabeceraExportar'])){ $dCabecera = $cabecera['cabeceraExportar']; }else{ $dCabecera = 'Reporte Direccionesclientes'; }
				...
				$dataProvider = $searchModel->search(Yii::$app->session['DireccionesclientesSearch']);
	...
	public function actionXportexcel($f){
		...
		if(isset(Yii::$app->session['DireccionesclientesSearch'])) {
			...
			$fID = Yii::$app->globals->getFormulario($f);
			$rauditoria = Yii::$app->globals->setRegistro(6, 'Excel', 'DireccionesClientes', $fID);
			...
			if(isset($catalogo['catalogoID'])){
				...
				header('Content-Disposition: attachment;filename="Direccionesclientes_'.date('ymdHis').'.xlsx"');
    ...
    /**
     * Lists all Direccionesclientes models.
     * @return mixed
     */
    public function actionIndex($f)
    {
		...
		if(isset($_GET['clear'])){
			Yii::$app->session['DireccionesclientesSearch'] = '';
			...
			if(count($params) <= 1){		  
				if(isset(Yii::$app->session['DireccionesclientesSearch'])) {
					$params = Yii::$app->session['DireccionesclientesSearch'];
				}else{
					Yii::$app->session['DireccionesclientesSearch'] = $params;
				}
			}else{	
				if(isset(Yii::$app->request->queryParams['DireccionesclientesSearch'])){
					Yii::$app->session['DireccionesclientesSearch'] = $params;
				}else{
					$params = Yii::$app->session['DireccionesclientesSearch'];
				}		
    ...
    /**
     * Displays a single Direccionesclientes model.
     * @param integer $id
     * @return mixed
	...
	public function actionCreateform()
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'DireccionesClientes', '1');
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="112"')->queryAll();
    ...
    /**
     * Creates a new Direccionesclientes model.
    ...
    public function actionCreate($f)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(3, 'Alta', 'DireccionesClientes', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="112"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['create', 'f'=>$f, 'insert' => 'true', 'id'=>$model->direccionClienteID]);
			}
		}
    ...
    /**
     * Updates an existing Direccionesclientes model.
    ...
    public function actionUpdate($f, $id)
    {
		...
		if($model->load(Yii::$app->request->post())) {
			...
			if($model->save()){
				...
				$rauditoria = Yii::$app->globals->setRegistro(5, 'Editar', 'DireccionesClientes', $fID);				
				$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="112"')->queryAll();
				foreach($registros as $row){
					Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);					
				}
				return $this->redirect(['update', 'f'=>$f, 'id' => $model->direccionClienteID, 'update'=>'true']);
			}
		}
	 ...
	 public function actionDeletedata($f, $id)
    {
		...
		if($model->save()){
			...
			$rauditoria = Yii::$app->globals->setRegistro(4, 'Eliminar', 'DireccionesClientes', $fID);				
			$registros = Yii::$app->db->createCommand('SELECT nombreCampo, campoPK FROM Campos WHERE catalogoID="112" and campoPK="1"')->queryAll();
			foreach($registros as $row){
				Yii::$app->globals->setRmodifica($rauditoria, $row['nombreCampo'], $model[$row['nombreCampo']]);
			}
    ...
    /**
     * Deletes an existing Direccionesclientes model.
    ...
    /**
     * Finds the Direccionesclientes model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Direccionesclientes the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Direccionesclientes::findOne($id)) !== null) {
            return $model;
        }
