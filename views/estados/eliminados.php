...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'estados', 'Estados', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Estados') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['estados/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->estadoID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'estadoID','label' => Yii::$app->globals->getTraductor('61', Yii::$app->session['idiomaId'], 'estadoID'),'value' => 'estadoID','format'=> 'raw'],
['attribute' => 'nombreEstado','label' => Yii::$app->globals->getTraductor('62', Yii::$app->session['idiomaId'], 'nombreEstado'),'value' => 'nombreEstado','format'=> 'raw'],
['attribute' => 'estadoEstado','label' => Yii::$app->globals->getTraductor('34', Yii::$app->session['idiomaId'], 'estadoEstado'),'value' => 'estadoEstado','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
['attribute' => 'paisID','label' => Yii::$app->globals->getTraductor('50', Yii::$app->session['idiomaId'], 'paisID'),'value' => 'idPaises.nombrePais','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=estados/update&f='.$_GET['f'].'&id='.$model->estadoID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=estados/delete",
                     data:{id:id},
