	...
	if(isset($_GET['insert'])){
		if($_GET['insert'] == 'true'){
			echo '<div class="alert alert-success" role="alert">
					 <i class="fa fa-check-square-o" aria-hidden="true"></i>  '.Yii::$app->globals->getTraductor(8, Yii::$app->session['idiomaId'], 'Registro actualizado con exito').'! ('.Html::a(Yii::$app->globals->getTraductor(15, Yii::$app->session['idiomaId'], 'Ver')." reportescampos", $url = ['reportescampos/update&f='.$_GET['f'].'&id='.$_GET['id']], $options = ['class'=>'']).')
				 </div>';
		}
	}
...
<?php 
$catalogo = Yii::$app->db->createCommand("SELECT * FROM Catalogos where regEstado='1' and nombreCatalogo='reportesCampos'")->queryOne();
			...
			if($rCampos['tipoControl'] == 'text' or $rCampos['tipoControl'] == 'number' or $rCampos['tipoControl'] == 'date' or $rCampos['tipoControl'] == 'email' or $rCampos['tipoControl'] == 'password' or $rCampos['tipoControl'] == 'color'){
				...
				echo $form->field($model, $rCampos['nombreCampo'],[
						...
						'pluginOptions' => [
							...
							'ajax' => [
								'url' => Url::to(['/reportescampos/getdatacombo']),
								'dataType' => 'json',
				...
				echo '<div class="col-sm-4 float-left pleft-0">
						<div class="form-group field-siniestros-d_detalle required has-success">
							...
							<select id="reportescampos-'.strtolower($rCampos['nombreCampo']).'" class="form-control" name="'.$catalogo['nombreModelo'].'['.$rCampos['nombreCampo'].']" required>';
