...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'infracciones', 'Infracciones', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Infracciones') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['infracciones/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->infraccionID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'infraccionID','label' => Yii::$app->globals->getTraductor('1', Yii::$app->session['idiomaId'], 'infraccionID'),'value' => 'infraccionID','format'=> 'raw'],
['attribute' => 'accidenteID','label' => Yii::$app->globals->getTraductor('725', Yii::$app->session['idiomaId'], 'accidenteID'),'value' => 'idAccidentes.descripcion','format'=> 'raw'],
...
['attribute' => 'tipoInfraccion','label' => Yii::$app->globals->getTraductor('726', Yii::$app->session['idiomaId'], 'tipoInfraccion'),'value' => 'tipoInfraccion','format'=> 'raw'],
['attribute' => 'montoMulta','label' => Yii::$app->globals->getTraductor('727', Yii::$app->session['idiomaId'], 'montoMulta'),'value' => 'montoMulta','format'=> 'raw'],
['attribute' => 'fechaInfraccion','label' => Yii::$app->globals->getTraductor('728', Yii::$app->session['idiomaId'], 'fechaInfraccion'),'value' => 'fechaInfraccion','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=infracciones/update&f='.$_GET['f'].'&id='.$model->infraccionID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=infracciones/delete",
                     data:{id:id},
