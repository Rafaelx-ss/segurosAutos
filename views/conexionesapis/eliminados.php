...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'conexionesapis', 'Conexionesapis', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Conexionesapis') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['conexionesapis/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->conexionApiID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'conexionApiID','label' => Yii::$app->globals->getTraductor('345', Yii::$app->session['idiomaId'], 'conexionApiID'),'value' => 'conexionApiID','format'=> 'raw'],
['attribute' => 'aplicacionConexionApiID','label' => Yii::$app->globals->getTraductor('342', Yii::$app->session['idiomaId'], 'aplicacionConexionApiID'),'value' => 'idAplicacionesconexionapi.descripcion','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=conexionesapis/update&f='.$_GET['f'].'&id='.$model->conexionApiID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=conexionesapis/delete",
                     data:{id:id},
