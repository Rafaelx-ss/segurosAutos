...
<?php 
$catalogo = Yii::$app->globals->getCatalogo('UsuariosAPI'); 
if(isset($catalogo['catalogoID'])){	
	...
	foreach($campos as $rCampos){
		if($rCampos['visible'] == 1){		
			if($rCampos['tipoControl'] == 'text' or $rCampos['tipoControl'] == 'number' or $rCampos['tipoControl'] == 'date' or $rCampos['tipoControl'] == 'email' or $rCampos['tipoControl'] == 'color'){
				...
				echo '</div>';
			}elseif($rCampos['tipoControl'] == 'password'){
				echo '<div class="col-4 float-left  pleft-0">';
				if($rCampos['campoRequerido'] == 1){
					echo $form->field($model, $rCampos['nombreCampo'])->textInput(['value'=>$rCampos['valorDefault'], 'type'=>$rCampos['tipoControl'], 'required'=>'required'])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				}else{
