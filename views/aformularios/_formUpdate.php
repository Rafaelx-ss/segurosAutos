	...
	if(isset($_GET['insert'])){
		if($_GET['insert'] == 'true'){
			echo '<div class="alert alert-success" role="alert">
					 <i class="fa fa-check-square-o" aria-hidden="true"></i>  Registro guardado con exito! ('.Html::a("Ver aformularios", $url = ['aformularios/update&id='.$_GET['id']], $options = ['class'=>'']).')
				 </div>';
		}
	}
...
<div class="col-4 float-left  pleft-0">
	<?= $form->field($model, 'claveAccion')->textInput(['required' => 'required']) ?>
</div>
...
<div class="col-sm-4 float-left pleft-0">  
	<?php 
		echo $form->field($model, 'accionID')->widget(Select2::classname(), [                         
			'data' => ArrayHelper::map(Acciones::find()->andWhere(['=', 'regEstado', '1'])->orderBy('nombreAccion')->all(), 'accionID', 'nombreAccion'),
			'language' => 'es',
			'options' => ['placeholder' => ' --- Selecciona --- '],
			'pluginOptions' => ['allowClear' => true]]); 	
	?>
