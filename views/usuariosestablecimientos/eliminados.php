...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'usuariosestablecimientos', 'Usuariosestablecimientos', $frmSeguridad, 'index', '0');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Usuariosestablecimientos') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['usuariosestablecimientos/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				
						['attribute' => 'usuarioID','label' => Yii::$app->globals->getTraductor('69', Yii::$app->session['idiomaId'], 'usuarioID'),'value' => 'idUsuarios.usuario','format'=> 'raw'],
['attribute' => 'establecimientoID','label' => Yii::$app->globals->getTraductor('535', Yii::$app->session['idiomaId'], 'establecimientoID'),'value' => 'idEstablecimientos.razonSocialEstablecimiento','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=usuariosestablecimientos/update&f='.$_GET['f'].'&usuarioID='.$model->usuarioID.'&establecimientoID='.$model->establecimientoID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=usuariosestablecimientos/delete",
                     data:{id:id},
