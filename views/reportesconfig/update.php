	...
	<div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
			...
			<?php 	
				...
				//Consulta
					if($perCons == 1){
						echo Html::a('<i class="'.$iSeacrh.'"></i> '.Yii::$app->globals->getTraductor(18, Yii::$app->session['idiomaId'], 'Consulta'), $url = ['reportesconfig/index&f='.$_GET['f']], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);
					}
					//Alta
					if($perAlta == 1){
						echo Html::a('<i class="'.$iAdd.'" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(2, Yii::$app->session['idiomaId'], 'Alta'), $url = ['reportesconfig/create&f='.$_GET['f']], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);			
					}
					//Eliminar
					if($perElim == 1){
						echo Html::a('<i class="'.$iDelete.'" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(3, Yii::$app->session['idiomaId'], 'Eliminar'), $url = ['reportesconfig/deletedata&f='.$_GET['f'].'&id='.$model->reporteConfiguracionID], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);
					}
					
					echo Html::a('<i class="fa fa-tasks"></i> Campos', $url = ['reportescampos/index&f='.$_GET['f'].'&token='.$model->reporteConfiguracionID], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);
					
					
					echo Html::a('<i class="ion-ios-refresh" aria-hidden="true"></i> Actualizar campos', $url = ['reportesconfig/updatecampos&f='.$_GET['f'].'&id='.$model->reporteConfiguracionID], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);
