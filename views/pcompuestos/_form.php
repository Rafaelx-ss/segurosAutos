	...
	if(isset($_GET['insert'])){
		if($_GET['insert'] == 'true'){
			echo '<div class="alert alert-success" role="alert">
					 <i class="fa fa-check-square-o" aria-hidden="true"></i>  Registro guardado con exito! ('.Html::a("Ver perfil compuestos", $url = ['pcompuestos/update&id='.$_GET['id']], $options = ['class'=>'']).')
				 </div>';
		}
	}
...
<div class="col-4 float-left  pleft-0">
	<?php 
		echo $form->field($model, 'usuarioID')->widget(Select2::classname(), [                         
			'data' => ArrayHelper::map(Usuarios::find()->andWhere(['=', 'regEstado', '1'])->orderBy('usuario')->all(), 'usuarioID', 'usuario'),
			'language' => 'es',
...
<div class="col-4 float-left  pleft-0">
	...
	if($model->isNewRecord){
		echo $form->field($model, 'activoPermiso')->checkbox(['checked'=>'checked']);
	}else{
		echo $form->field($model, 'activoPermiso')->checkbox();
	}
	?>
</div>
