...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'metodosapis', 'Metodosapis', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Metodosapis') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['metodosapis/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->metodoApiID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'metodoApiID','label' => Yii::$app->globals->getTraductor('91', Yii::$app->session['idiomaId'], 'metodoApiID'),'value' => 'metodoApiID','format'=> 'raw'],
['attribute' => 'apiID','label' => Yii::$app->globals->getTraductor('90', Yii::$app->session['idiomaId'], 'apiID'),'value' => 'idApis.nombreApi','format'=> 'raw'],
['attribute' => 'estadoMetodo','label' => Yii::$app->globals->getTraductor('34', Yii::$app->session['idiomaId'], 'estadoMetodo'),'value' => 'estadoMetodo','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
['attribute' => 'tipoMetodoApi','label' => Yii::$app->globals->getTraductor('91', Yii::$app->session['idiomaId'], 'tipoMetodoApi'),'value' => 'tipoMetodoApi','format'=> 'raw'],
['attribute' => 'permisoMaster','label' => Yii::$app->globals->getTraductor('323', Yii::$app->session['idiomaId'], 'permisoMaster'),'value' => 'permisoMaster','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
['attribute' => 'permisoGrupoEstablecimiento','label' => Yii::$app->globals->getTraductor('324', Yii::$app->session['idiomaId'], 'permisoGrupoEstablecimiento'),'value' => 'permisoGrupoEstablecimiento','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
['attribute' => 'permisoEstablecimiento','label' => Yii::$app->globals->getTraductor('325', Yii::$app->session['idiomaId'], 'permisoEstablecimiento'),'value' => 'permisoEstablecimiento','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=metodosapis/update&f='.$_GET['f'].'&id='.$model->metodoApiID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=metodosapis/delete",
                     data:{id:id},
