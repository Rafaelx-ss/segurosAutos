...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'aplicaciones', 'Aplicaciones', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Aplicaciones') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['aplicaciones/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->aplicacionID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'aplicacionID','label' => Yii::$app->globals->getTraductor('98', Yii::$app->session['idiomaId'], 'aplicacionID'),'value' => 'aplicacionID','format'=> 'raw'],
['attribute' => 'nombreAplicacion','label' => Yii::$app->globals->getTraductor('96', Yii::$app->session['idiomaId'], 'nombreAplicacion'),'value' => 'nombreAplicacion','format'=> 'raw'],
['attribute' => 'activoAplicacion','label' => Yii::$app->globals->getTraductor('34', Yii::$app->session['idiomaId'], 'activoAplicacion'),'value' => 'activoAplicacion','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=aplicaciones/update&f='.$_GET['f'].'&id='.$model->aplicacionID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=aplicaciones/delete",
                     data:{id:id},
