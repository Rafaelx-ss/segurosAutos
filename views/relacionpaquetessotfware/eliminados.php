...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'relacionpaquetessotfware', 'Relacionpaquetessotfware', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Relacionpaquetessotfware') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['relacionpaquetessotfware/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->relacionPaqueteSotfwareID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'relacionPaqueteSotfwareID','label' => Yii::$app->globals->getTraductor('392', Yii::$app->session['idiomaId'], 'relacionPaqueteSotfwareID'),'value' => 'relacionPaqueteSotfwareID','format'=> 'raw'],
['attribute' => 'paqueteKairosID','label' => Yii::$app->globals->getTraductor('381', Yii::$app->session['idiomaId'], 'paqueteKairosID'),'value' => 'idPaqueteskairos.nombrePaquete','format'=> 'raw'],
['attribute' => 'softwareKairosID','label' => Yii::$app->globals->getTraductor('389', Yii::$app->session['idiomaId'], 'softwareKairosID'),'value' => 'idSoftwarekairos.nombreSoftware','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=relacionpaquetessotfware/update&f='.$_GET['f'].'&id='.$model->relacionPaqueteSotfwareID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=relacionpaquetessotfware/delete",
                     data:{id:id},
