...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'versiones', 'Versiones', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Versiones') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['versiones/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->versionID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'versionID','label' => Yii::$app->globals->getTraductor('185', Yii::$app->session['idiomaId'], 'versionID'),'value' => 'versionID','format'=> 'raw'],
['attribute' => 'version','label' => Yii::$app->globals->getTraductor('186', Yii::$app->session['idiomaId'], 'version'),'value' => 'version','format'=> 'raw'],
['attribute' => 'fechaLiberacionVersion','label' => Yii::$app->globals->getTraductor('187', Yii::$app->session['idiomaId'], 'fechaLiberacionVersion'),'value' => 'fechaLiberacionVersion','format'=> 'raw'],
['attribute' => 'aliasVersion','label' => Yii::$app->globals->getTraductor('188', Yii::$app->session['idiomaId'], 'aliasVersion'),'value' => 'aliasVersion','format'=> 'raw'],
['attribute' => 'urlVersion','label' => Yii::$app->globals->getTraductor('189', Yii::$app->session['idiomaId'], 'urlVersion'),'value' => 'urlVersion','format'=> 'raw'],
['attribute' => 'urlDocumentacionVersion','label' => Yii::$app->globals->getTraductor('190', Yii::$app->session['idiomaId'], 'urlDocumentacionVersion'),'value' => 'urlDocumentacionVersion','format'=> 'raw'],
...
['attribute' => 'aplicacionID','label' => Yii::$app->globals->getTraductor('96', Yii::$app->session['idiomaId'], 'aplicacionID'),'value' => 'idAplicaciones.nombreAplicacion','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=versiones/update&f='.$_GET['f'].'&id='.$model->versionID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=versiones/delete",
                     data:{id:id},
