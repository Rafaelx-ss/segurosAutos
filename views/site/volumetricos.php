	...
	<div class="card-body">	
		...
		<div style="text-align: right;">
			...
			$form = ActiveForm::begin([
				'id' => 'login-form',
				'action' => ['site/volumetricos&f='.$_GET['f']],
		...
		if(isset($_POST['FechaInicial']) and isset($_POST['FechaFinal']) and isset($_POST['Siic'])){
			...
			if(isset($clApp3['usuario'])){
				$UsernameCentral = $clApp3['usuario'];
				openssl_private_decrypt(base64_decode($clApp3['password']), $PasswordPOSTdec, $llaveprivada);
				$PasswordCentral = $PasswordPOSTdec;
				$RutaApiCentral = $clApp3['rutaApi'];
				$ptemp0 = $clApp3['password'];
			}
			echo "Ruta BonoboGrupo Envio:".$RutaApiCentral."/api/ApiToken.php<br>";
			...
			if(isset($clApp2['usuario'])){
				$Username = $clApp2['usuario'];
				openssl_private_decrypt(base64_decode($clApp2['password']), $PasswordPOSTsqldec, $llaveprivada);
				$Password = $PasswordPOSTsqldec;
				$RutaApiLocal = $clApp2['rutaApi'];
				$ptemp1 = $clApp2['password'];
			}
			
		
			echo "Ruta BonoboLocal:".$RutaApiLocal."/api/ApiToken.php<br>";
			...
			if($_POST['FechaInicial'] <= $_POST['FechaFinal']){				
				...
				ApiHttpClient::Init($RutaApiLocal);
				...
				try{
					if(ApiHttpClient::$Resultado){
						$TokenApiLocal = ApiHttpClient::$Token;
					}else{
				...
				ApiHttpClient::Init($RutaApiCentral);
				ApiHttpClient::$UserName = $UsernameCentral;
				ApiHttpClient::$Password = $PasswordCentral;
				ApiHttpClient::$MacAddress =  $dataMacAddress;
				...
				if($TokenApiLocal != "" and $TokenApiMysql != "" ){
					while(strtotime($FechaFinWhile) >= strtotime($FechaInicio)) { 
						...
						if($apiSel=='all' or $apiSel=='RecepcionesCargas'){
							...
							ApiHttpClient::$Password = $Password;
							ApiHttpClient::$MacAddress =  $dataMacAddress;
							ApiHttpClient::$Token = $TokenApiLocal;
							...
							$datos = ApiHttpClient::ConsumeApi('/api/ApiRecepcionesCargasBonobo.php','GET',$parametros);
							...
							if(ApiHttpClient::$EstatusHttp == 200){
								if(isset($arraydatos['resultado'])){
									...
									if($total_datos == 0){
										...
										$apiFactResp = ApiHttpClient::ConsumeApi('/api/ApiRecepcionesCargasBonobo.php','POST',$parametrosMysql);
						...
						if($apiSel=='all' or $apiSel=='DocumentosCargas'){
							...
							$datos = ApiHttpClient::ConsumeApi('/api/ApiDocumentosCargasBonobo.php','GET',$parametros);
							$arraydatos = json_decode($datos,true);
							...
							if(ApiHttpClient::$EstatusHttp == 200){
								if(isset($arraydatos['resultado'])){
									...
									if($total_datos == 0){
										...
										$apiFactResp = ApiHttpClient::ConsumeApi('/api/ApiDocumentosCargasBonobo.php','POST',$parametrosMysql);
										$arrayApiFac = json_decode($apiFactResp,true);
						...
						if($apiSel=='all' or $apiSel=='LecturasTanques'){
							...
							$cabecera .= "<strong>Existencias Tanques del ".$FechaInicioMysql." al ".$FechaFinMysql."</strong><br>";
							$tiempoEjecuta .= "Fecha inicio : ".$tiempoEjecutaInicio;
							...
							$datos = ApiHttpClient::ConsumeApi('/api/ApiExistenciasTanquesBonobo.php','GET',$parametros);
							$arraydatos = json_decode($datos,true);
							...
							if(ApiHttpClient::$EstatusHttp == 200){
								if(isset($arraydatos['resultado'])){
									...
									if($total_datos == 0){
										...
										$apiFactResp = ApiHttpClient::ConsumeApi('/api/ApiExistenciasTanquesBonobo.php','POST',$parametrosMysql);
										$arrayApiFac = json_decode($apiFactResp,true);
						...
						if($apiSel=='all' or $apiSel=='Jornadas'){
							...
							$datos = ApiHttpClient::ConsumeApi('/api/ApiJornadasBonobo.php','GET',$parametros);
							$arraydatos = json_decode($datos,true);
							...
							if(ApiHttpClient::$EstatusHttp == 200){
								if(isset($arraydatos['resultado'])){
									...
									if($total_datos == 0){
										...
										$apiFactResp = ApiHttpClient::ConsumeApi('/api/ApiJornadasBonobo.php','POST',$parametrosMysql);
										$arrayApiFac = json_decode($apiFactResp,true);
						...
						if($apiSel=='all' or $apiSel=='Asistencias'){
							...
							$datos = ApiHttpClient::ConsumeApi('/api/ApiAsistenciasBonobo.php','GET',$parametros);
							$arraydatos = json_decode($datos,true);
							...
							if(ApiHttpClient::$EstatusHttp == 200){
								if(isset($arraydatos['resultado'])){
									...
									if($total_datos == 0){
										...
										$apiFactResp = ApiHttpClient::ConsumeApi('/api/ApiAsistenciasBonobo.php','POST',$parametrosMysql);
										$arrayApiFac = json_decode($apiFactResp,true);
						...
						if($apiSel=='all' or $apiSel=='Despachos'){
							...
							$datos = ApiHttpClient::ConsumeApi('/api/ApiVentasCombustiblesBonobo.php','GET',$parametros);
							$arraydatos = json_decode($datos,true);
							...
							if(ApiHttpClient::$EstatusHttp == 200){
								if(isset($arraydatos['resultado'])){
									...
									if($total_datos == 0){
										...
										$apiFactResp = ApiHttpClient::ConsumeApi('/api/ApiVentasCombustiblesBonobo.php','POST',$parametrosMysql);
										$arrayApiFac = json_decode($apiFactResp,true);
								...
								$date21 = new DateTime($tiempoEjecutafinalPost);
								$diff1 = $date11->diff($date21);	
								$tiempoEjecutaPost .= " | Fecha final : ".$tiempoEjecutafinalPost.$mDesarrollo10;
							}else{
