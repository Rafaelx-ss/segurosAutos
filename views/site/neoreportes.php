		...
		<div style="text-align: right;">
			...
			<div class="form-group" style="margin-right: 20px;">
				...
				<select name="tipo" required class="form-control input-sm">
					...
					<?php
					if($tipoDataSel == "D"){
						echo '<option value="D" selected="selected"> Diario</option>
			...
			if(isset($existeArchivo['reporteConVolID'])){
				if(isset($_POST['updateFile'])){
					if(isset($_POST['reporteConVolID'])){
						...
						echo "Ruta Api 1:".$RutaApiMysql.'/api/ApiToken.php';
						echo "<br>";
										...
										if($generado == true){
											...
											echo "<a href='../reportes/".$arraydatos['datos']['nombreArchivoReporteConVol']."'>Descargar Archivo</a>";
											 ...
											 echo '<div class="alert alert-warning" role="alert">
													No pudimos generar el archivo'.$mDesarrollo.'
											</div>';
										}
									}else{
										 echo '<div class="alert alert-warning" role="alert">
													No se obtuvieron los datos para generar el archivo '.$mDesarrollo.'
											</div>'; 
									}						

								}else{
									 echo '<div class="alert alert-warning" role="alert">
													Ocurrio un error al generar el archivo '.$datos.$mDesarrollo.'
											</div>'; 
								}
							}else{
								 echo '<div class="alert alert-warning" role="alert">
							...
							if(isset($arraydatos['datos']['nombreArchivoReporteConVol'])){
								...
								if($generado == true){
									...
									echo "<a href='../reportes/".$arraydatos['datos']['nombreArchivoReporteConVol']."'>Descargar Archivo</a>";
									 ...
									 echo '<div class="alert alert-warning" role="alert">
											No pudimos generar el archivo '.$mDesarrollo2.'
									</div>';
								}
							}else{
								 echo '<div class="alert alert-warning" role="alert">
											No se obtuvieron los datos para generar el archivo  '.$mDesarrollo2.'
									</div>'; 
							}						

						}else{
							 echo '<div class="alert alert-warning" role="alert">
											Ocurrio un error al generar el archivo: '.$datos.$mDesarrollo2.'
									</div>'; 
						}
					}else{
						 echo '<div class="alert alert-warning" role="alert">
...
function guarda_archivo($datos, $nombre_archivo){ 
	...
	$json =  json_encode($datos, JSON_NUMERIC_CHECK | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
