...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'municipios', 'Municipios', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Municipios') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['municipios/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->municipioID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'municipioID','label' => Yii::$app->globals->getTraductor('1', Yii::$app->session['idiomaId'], 'municipioID'),'value' => 'municipioID','format'=> 'raw'],
['attribute' => 'nombreMunicipio','label' => Yii::$app->globals->getTraductor('694', Yii::$app->session['idiomaId'], 'nombreMunicipio'),'value' => 'nombreMunicipio','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=municipios/update&f='.$_GET['f'].'&id='.$model->municipioID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=municipios/delete",
                     data:{id:id},
