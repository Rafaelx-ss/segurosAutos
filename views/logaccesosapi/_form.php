...
<?php 
$catalogo = Yii::$app->db->createCommand("SELECT * FROM Catalogos where regEstado='1' and nombreCatalogo='LogAccesosAPI'")->queryOne();
		...
		if($rCampos['visible'] == 1){		
			if($rCampos['tipoControl'] == 'text' or $rCampos['tipoControl'] == 'number' or $rCampos['tipoControl'] == 'date' or $rCampos['tipoControl'] == 'email' or $rCampos['tipoControl'] == 'password' or $rCampos['tipoControl'] == 'color'){
				...
				if($rCampos['campoRequerido'] == 1){
					echo $form->field($model, $rCampos['nombreCampo'])->textInput(['value'=>$defaulValue, 'type'=>$rCampos['tipoControl'], 'required'=>'required'])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				}else{
					echo $form->field($model, $rCampos['nombreCampo'])->textInput(['value'=>$defaulValue, 'type'=>$rCampos['tipoControl']])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				}
				...
				if($rCampos['tipoControl'] == 'date'){$defaulValue = date('Y-m-d');}else{$defaulValue = $rCampos['valorDefault'];}
				if($rCampos['campoRequerido'] == 1){
					echo $form->field($model, $rCampos['nombreCampo'])->textInput(['value'=>$defaulValue, 'type'=>'number', 'required'=>'required', 'step'=>'any'])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				}else{
					echo $form->field($model, $rCampos['nombreCampo'])->textInput(['value'=>$defaulValue, 'type'=>'number', 'step'=>'any'])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				}
				...
				echo $form->field($model, $rCampos['nombreCampo'],[
						...
						'pluginOptions' => [
							...
							'ajax' => [
								'url' => Url::to(['/logaccesosapi/getdatacombo']),
								'dataType' => 'json',
				...
				echo '<div class="col-sm-4 float-left pleft-0">
						<div class="form-group field-siniestros-d_detalle required has-success">
							...
							<select id="logaccesosapi-'.strtolower($rCampos['nombreCampo']).'" class="form-control" name="'.$catalogo['nombreModelo'].'['.$rCampos['nombreCampo'].']" required>
				...
				echo '<div class="col-4 float-left  pleft-0">';
				echo $form->field($model, $rCampos['nombreCampo'])->textInput(['value'=>$rCampos['valorDefault']])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				echo '</div>';
			}
		}else{
