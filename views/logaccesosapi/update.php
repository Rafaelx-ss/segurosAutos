	...
	<div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
			...
			echo '<div style="margin-top: 20px; float: right;">';
					//Consulta
					if($perCons == 1){
						echo Html::a('<i class="'.$iSeacrh.'"></i> '.Yii::$app->globals->getTraductor(18, Yii::$app->session['idiomaId'], 'Consulta'), $url = ['logaccesosapi/index&f='.$_GET['f']], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);
					}
					//Alta
					if($perAlta == 1){
						echo Html::a('<i class="'.$iAdd.'" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(2, Yii::$app->session['idiomaId'], 'Alta'), $url = ['logaccesosapi/create&f='.$_GET['f']], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);			
					}
					//Eliminar
					if($perElim == 1){
						echo Html::a('<i class="'.$iDelete.'" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(3, Yii::$app->session['idiomaId'], 'Eliminar'), $url = ['logaccesosapi/deletedata&f='.$_GET['f'].'&logAccesoApiId='.$model->logAccesoApiId.'&establecimientoID='.$model->establecimientoID], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);
					}
			echo '</div>';
