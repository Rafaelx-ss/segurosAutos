	...
	<div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
			...
			echo '<div style="margin-top: 20px; float: right;">';
					...
					if($perExcel == 1){
						echo Html::a('<i class="'.$iExcel.'"></i> '.Yii::$app->globals->getTraductor(22, Yii::$app->session['idiomaId'], 'Excel'), $url = ['logaccesosapi/xportexcel&f='.$idForm], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones()]);
					}
					
					if($perPdf == 1){
						echo Html::a('<i class="'.$iPdf.'"></i> '.Yii::$app->globals->getTraductor(23, Yii::$app->session['idiomaId'], 'PDF'), $url = ['logaccesosapi/xportpdf&f='.$idForm], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn '.Yii::$app->globals->btnAcciones(), 'target'=>'_blank']);
					}
			echo '</div>';
...
<?php 
$catalogoIndex = Yii::$app->db->createCommand("SELECT * FROM Catalogos where regEstado='1' and nombreCatalogo='LogAccesosAPI'")->queryOne();
$arrayGrid = array();
$showSearch = 0;
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['logaccesosapi/index&f='.$_GET['f'].'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				
						['attribute' => 'logAccesoApiId','label' => Yii::$app->globals->getTraductor('230', Yii::$app->session['idiomaId'], 'logAccesoApiId'),'value' => 'logAccesoApiId','format'=> 'raw'],
['attribute' => 'establecimientoID','label' => Yii::$app->globals->getTraductor('41', Yii::$app->session['idiomaId'], 'establecimientoID'),'value' => 'idEstablecimientos.aliasEstablecimiento','format'=> 'raw'],
['attribute' => 'usuarioApiID','label' => Yii::$app->globals->getTraductor('69', Yii::$app->session['idiomaId'], 'usuarioApiID'),'value' => 'idUsuariosapi.nombreUsuario','format'=> 'raw'],
['attribute' => 'ipAddress','label' => Yii::$app->globals->getTraductor('84', Yii::$app->session['idiomaId'], 'ipAddress'),'value' => 'ipAddress','format'=> 'raw'],
['attribute' => 'macAddress','label' => Yii::$app->globals->getTraductor('85', Yii::$app->session['idiomaId'], 'macAddress'),'value' => 'macAddress','format'=> 'raw'],
['attribute' => 'fechaAcceso','label' => Yii::$app->globals->getTraductor('47', Yii::$app->session['idiomaId'], 'fechaAcceso'),'value' => 'fechaAcceso','format'=> 'raw'],
['attribute' => 'nombreApi','label' => Yii::$app->globals->getTraductor('90', Yii::$app->session['idiomaId'], 'nombreApi'),'value' => 'nombreApi','format'=> 'raw'],
['attribute' => 'codigoResultado','label' => Yii::$app->globals->getTraductor('1', Yii::$app->session['idiomaId'], 'codigoResultado'),'value' => 'codigoResultado','format'=> 'raw'],
['attribute' => 'mensajeResultado','label' => Yii::$app->globals->getTraductor('231', Yii::$app->session['idiomaId'], 'mensajeResultado'),'value' => 'mensajeResultado','format'=> 'raw'],
['attribute' => 'parametrosApi','label' => Yii::$app->globals->getTraductor('232', Yii::$app->session['idiomaId'], 'parametrosApi'),'value' => 'parametrosApi','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=logaccesosapi/update&f='.$_GET['f'].'&logAccesoApiId='.$model->logAccesoApiId.'&establecimientoID='.$model->establecimientoID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=logaccesosapi/delete",
                     data:{id:id},
