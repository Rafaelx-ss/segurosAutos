...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'contactosclientes', 'Contactosclientes', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Contactosclientes') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['contactosclientes/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->contactoClienteID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'contactoClienteNombre','label' => Yii::$app->globals->getTraductor('484', Yii::$app->session['idiomaId'], 'contactoClienteNombre'),'value' => 'contactoClienteNombre','format'=> 'raw'],
['attribute' => 'clienteID','label' => Yii::$app->globals->getTraductor('509', Yii::$app->session['idiomaId'], 'clienteID'),'value' => 'clienteID','format'=> 'raw'],
['attribute' => 'email','label' => Yii::$app->globals->getTraductor('72', Yii::$app->session['idiomaId'], 'email'),'value' => 'email','format'=> 'raw'],
['attribute' => 'telefono','label' => Yii::$app->globals->getTraductor('290', Yii::$app->session['idiomaId'], 'telefono'),'value' => 'telefono','format'=> 'raw'],
['attribute' => 'celular','label' => Yii::$app->globals->getTraductor('291', Yii::$app->session['idiomaId'], 'celular'),'value' => 'celular','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=contactosclientes/update&f='.$_GET['f'].'&id='.$model->contactoClienteID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=contactosclientes/delete",
                     data:{id:id},
