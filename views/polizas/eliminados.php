...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'polizas', 'Polizas', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Polizas') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['polizas/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->polizaID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'polizaID','label' => Yii::$app->globals->getTraductor('1', Yii::$app->session['idiomaId'], 'polizaID'),'value' => 'polizaID','format'=> 'raw'],
...
['attribute' => 'numeroPoliza','label' => Yii::$app->globals->getTraductor('714', Yii::$app->session['idiomaId'], 'numeroPoliza'),'value' => 'numeroPoliza','format'=> 'raw'],
['attribute' => 'montoCobertura','label' => Yii::$app->globals->getTraductor('715', Yii::$app->session['idiomaId'], 'montoCobertura'),'value' => 'montoCobertura','format'=> 'raw'],
['attribute' => 'costoPoliza','label' => Yii::$app->globals->getTraductor('716', Yii::$app->session['idiomaId'], 'costoPoliza'),'value' => 'costoPoliza','format'=> 'raw'],
['attribute' => 'fechaCompra','label' => Yii::$app->globals->getTraductor('717', Yii::$app->session['idiomaId'], 'fechaCompra'),'value' => 'fechaCompra','format'=> 'raw'],
['attribute' => 'fechaVencimiento','label' => Yii::$app->globals->getTraductor('718', Yii::$app->session['idiomaId'], 'fechaVencimiento'),'value' => 'fechaVencimiento','format'=> 'raw'],
['attribute' => 'estadoPoliza','label' => Yii::$app->globals->getTraductor('719', Yii::$app->session['idiomaId'], 'estadoPoliza'),'value' => 'estadoPoliza','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=polizas/update&f='.$_GET['f'].'&id='.$model->polizaID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=polizas/delete",
                     data:{id:id},
