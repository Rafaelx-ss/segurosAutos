	...
	<div class="page-title-wrapper">
    	<div class="page-title-heading">
        	<div class="page-title-icon">
            	<i class="pe-7s-copy-file icon-gradient bg-malibu-beach"></i>
            </div>
            <div>
				Tipo de formularios				<div class="page-title-subheading">Consulta</div>
            </div>
        </div>
        <div class="page-title-actions">
			...
			foreach($iconAcciones as $rAccion){
				...
				if($rAccion['accionID'] == '7'){
					...
					echo Html::a('<i class="'.$iSeacrh.'"></i> Consulta', $url = ['tformularios/index'], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn btn-outline-info']);
			
			
					echo Html::a('<i class="'.$iAdd.'" aria-hidden="true"></i> Alta', $url = ['tformularios/create'], $options = ['class'=>'mb-2 mr-2 btn-pill btn-transition btn btn-outline-info']);
			
					echo  Html::button('<i class="'.$iDelete.'" aria-hidden="true"></i> Eliminar', ['class' => 'mb-2 mr-2 btn-pill btn-transition btn btn-outline-info', 'onclick' => 'getRows()']);
			?>
		</div>
     </div>
</div>
...
<div class="main-card mb-3 card">
      <div class="card-body">				
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> Limpiar filtro', $url = ['tformularios/index&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],		  				
						['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->tipoFormularioID];
								},
						],
            //'tipoFormularioID',
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=tformularios/update&id='.$model->tipoFormularioID;
											return Html::a(
										...
										'delete' => function ($url, $model) {
											$url_group = 'javascript:confirmDelete('.$model->tipoFormularioID.')';
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('Confirmación', '¿Seguro que desea eliminar el registro?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=tformularios/delete",
                     data:{id:id},
