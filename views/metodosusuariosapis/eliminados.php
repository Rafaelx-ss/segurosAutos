...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'metodosusuariosapis', 'Metodosusuariosapis', $frmSeguridad, 'index', '0');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Metodosusuariosapis') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['metodosusuariosapis/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				
						['attribute' => 'usuarioApiID','label' => Yii::$app->globals->getTraductor('69', Yii::$app->session['idiomaId'], 'usuarioApiID'),'value' => 'idUsuariosapi.usuario','format'=> 'raw'],
['attribute' => 'metodoApiID','label' => Yii::$app->globals->getTraductor('92', Yii::$app->session['idiomaId'], 'metodoApiID'),'value' => function($model){
											$dataQury = Yii::$app->db->createCommand("Select MetodosApis.metodoApiID,CONCAT(Apis.nombreApi,' ',MetodosApis.tipoMetodoApi) as metodo from MetodosApis,Apis where MetodosApis.apiID=Apis.apiID and MetodosApis.metodoApiID='".$model['metodoApiID']."'")->queryOne();
											if(isset($dataQury['metodo'])){
												return $dataQury['metodo'];
											}else{
												return "N/A";
											}											
										},'format'=> 'raw'],
['attribute' => 'estadoDetalleApi','label' => Yii::$app->globals->getTraductor('34', Yii::$app->session['idiomaId'], 'estadoDetalleApi'),'value' => 'estadoDetalleApi','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=metodosusuariosapis/update&f='.$_GET['f'].'&usuarioApiID='.$model->usuarioApiID.'&metodoApiID='.$model->metodoApiID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=metodosusuariosapis/delete",
                     data:{id:id},
