...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'menuspos', 'Menuspos', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Menuspos') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['menuspos/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->menuPOSID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'menuPOSID','label' => Yii::$app->globals->getTraductor('169', Yii::$app->session['idiomaId'], 'menuPOSID'),'value' => 'menuPOSID','format'=> 'raw'],
['attribute' => 'nombreMenuPOS','label' => Yii::$app->globals->getTraductor('172', Yii::$app->session['idiomaId'], 'nombreMenuPOS'),'value' => 'nombreMenuPOS','format'=> 'raw'],
['attribute' => 'tipoPosID','label' => Yii::$app->globals->getTraductor('170', Yii::$app->session['idiomaId'], 'tipoPosID'),'value' => 'idTipospos.descripcionTipoPOS','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=menuspos/update&f='.$_GET['f'].'&id='.$model->menuPOSID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=menuspos/delete",
                     data:{id:id},
