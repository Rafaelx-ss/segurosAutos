	...
	if(isset($_GET['insert'])){
		if($_GET['insert'] == 'true'){
			echo '<div class="alert alert-success" role="alert">
					 <i class="fa fa-check-square-o" aria-hidden="true"></i>  '.Yii::$app->globals->getTraductor(8, Yii::$app->session['idiomaId'], 'Registro actualizado con exito').'! ('.Html::a(Yii::$app->globals->getTraductor(15, Yii::$app->session['idiomaId'], 'Ver')." apiconfiguraciones", $url = ['apiconfiguraciones/update&f='.$_GET['f'].'&id='.$_GET['id']], $options = ['class'=>'']).')
				 </div>';
		}
	}
...
<?php 
$catalogo = Yii::$app->db->createCommand("SELECT * FROM Catalogos where regEstado='1' and nombreCatalogo='apilistaconfiguraciones'")->queryOne();
	...
	foreach($campos as $rCampos){
		if($rCampos['visible'] == 1){		
			if($rCampos['tipoControl'] == 'text' or $rCampos['tipoControl'] == 'number' or $rCampos['tipoControl'] == 'date' or $rCampos['tipoControl'] == 'email' or $rCampos['tipoControl'] == 'password' or $rCampos['tipoControl'] == 'color'){
				...
				echo '<div class="col-4 float-left  pleft-0">';
				if($rCampos['campoRequerido'] == 1){
					echo $form->field($model, $rCampos['nombreCampo'])->textInput(['type'=>'number', 'required'=>'required', 'step'=>'any'])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				}else{
					echo $form->field($model, $rCampos['nombreCampo'])->textInput(['type'=>'number', 'step'=>'any'])->label(Yii::$app->globals->getTraductor(getNameInput($rCampos['textoID']), Yii::$app->session['idiomaId'], $rCampos['nombreCampo']));
				}
				...
				echo $form->field($model, $rCampos['nombreCampo'],[
						...
						'pluginOptions' => [
							...
							'ajax' => [
								'url' => Url::to(['/apiconfiguraciones/getdatacombo']),
								'dataType' => 'json',
				...
				$combo = Yii::$app->db->createCommand("SELECT CombosAnidados.controlQuery, CombosAnidados.campoIDdependiente, CombosAnidados.queryValue, CombosAnidados.queryText, Campos.nombreCampo, CombosAnidados.parametrosQuery FROM CombosAnidados inner join Campos on Campos.campoID = CombosAnidados.campoIDPadre where CombosAnidados.catalogoID='".$rCampos['catalogoID']."' and campoIDdependiente='".$rCampos['campoID']."'")->queryOne();
				...
				echo '<div class="col-sm-4 float-left pleft-0">
						<div class="form-group field-siniestros-d_detalle required has-success">
							...
							<select id="apiconfiguraciones-'.strtolower($rCampos['nombreCampo']).'" class="form-control" name="'.$catalogo['nombreModelo'].'['.$rCampos['nombreCampo'].']" required>';
