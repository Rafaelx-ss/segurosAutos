...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'mensajesbitacora', 'Mensajesbitacora', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Mensajesbitacora') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['mensajesbitacora/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->mensajeBitacoraID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'mensajeBitacoraID','label' => Yii::$app->globals->getTraductor('427', Yii::$app->session['idiomaId'], 'mensajeBitacoraID'),'value' => 'mensajeBitacoraID','format'=> 'raw'],
['attribute' => 'nombreMensaje','label' => Yii::$app->globals->getTraductor('428', Yii::$app->session['idiomaId'], 'nombreMensaje'),'value' => 'nombreMensaje','format'=> 'raw'],
['attribute' => 'activoMensaje','label' => Yii::$app->globals->getTraductor('34', Yii::$app->session['idiomaId'], 'activoMensaje'),'value' => 'activoMensaje','filter'=> [0=>'No',1=>'Si'],'format'=> 'boolean'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=mensajesbitacora/update&f='.$_GET['f'].'&id='.$model->mensajeBitacoraID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=mensajesbitacora/delete",
                     data:{id:id},
