...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'mensajesbitacoraidiomas', 'Mensajesbitacoraidiomas', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Mensajesbitacoraidiomas') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['mensajesbitacoraidiomas/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->mensajeBitacoraIdiomasID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'mensajeBitacoraIdiomasID','label' => Yii::$app->globals->getTraductor('430', Yii::$app->session['idiomaId'], 'mensajeBitacoraIdiomasID'),'value' => 'mensajeBitacoraIdiomasID','format'=> 'raw'],
['attribute' => 'mensajeBitacoraID','label' => Yii::$app->globals->getTraductor('428', Yii::$app->session['idiomaId'], 'mensajeBitacoraID'),'value' => 'idMensajesbitacora.nombreMensaje','format'=> 'raw'],
['attribute' => 'mensaje','label' => Yii::$app->globals->getTraductor('431', Yii::$app->session['idiomaId'], 'mensaje'),'value' => 'mensaje','format'=> 'raw'],
['attribute' => 'idiomaID','label' => Yii::$app->globals->getTraductor('433', Yii::$app->session['idiomaId'], 'idiomaID'),'value' => 'idIdiomas.nombreIdioma','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=mensajesbitacoraidiomas/update&f='.$_GET['f'].'&id='.$model->mensajeBitacoraIdiomasID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=mensajesbitacoraidiomas/delete",
                     data:{id:id},
