	...
	if(isset($_GET['insert'])){
		if($_GET['insert'] == 'true'){
			echo '<div class="alert alert-success" role="alert">
					 <i class="fa fa-check-square-o" aria-hidden="true"></i>  Registro guardado con exito! ('.Html::a("Ver perfil menu", $url = ['pmenus/update&id='.$_GET['id']], $options = ['class'=>'']).')
				 </div>';
		}
	}
...
<div class="col-4 float-left  pleft-0">
	...
	if($model->isNewRecord){
		echo $form->field($model, 'activoMenusFormularios')->checkbox(['checked'=>'checked']);
	}else{
		echo $form->field($model, 'activoMenusFormularios')->checkbox();
	}
	?>
</div>
