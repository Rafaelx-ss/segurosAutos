...
$perElimckh = false;
$menuBotones = Yii::$app->globals->getActionButton($idForm, 'reportespdf', 'Reportespdf', $frmSeguridad, 'index', '1');

$perElimckh = $menuBotones['visible'];
echo $menuBotones['botones'];

if(Yii::$app->globals->getIndexSearch('Reportespdf') != 0){
...
<div class="main-card mb-3 card">
	...
	?>
					...
					'summary' => Html::a('<i class="fa fa-minus-square" aria-hidden="true"></i> '.Yii::$app->globals->getTraductor(5, Yii::$app->session['idiomaId'], 'Limpiar filtros'), $url = ['reportespdf/eliminados&f='.$idForm.'&clear=true'], $options = ['style'=>'color:#003e59;']).' | Viendo {count} de {totalCount} resultados.', 
					'filterModel' => $searchModel,       
					'columns' => [
						['class' => 'yii\grid\SerialColumn'],
		  						  				['class' => 'yii\grid\CheckboxColumn', 'checkboxOptions' => function($data) {
										return ['value' => $data->reportesPdfID];
								}, 'visible' => $perElimckh,
						],
		  						  				
						['attribute' => 'reportesPdfID','label' => Yii::$app->globals->getTraductor('1', Yii::$app->session['idiomaId'], 'reportesPdfID'),'value' => 'reportesPdfID','format'=> 'raw'],
['attribute' => 'establecimientoID','label' => Yii::$app->globals->getTraductor('40', Yii::$app->session['idiomaId'], 'establecimientoID'),'value' => 'idEstablecimientos.aliasEstablecimiento','format'=> 'raw'],
['attribute' => 'folioReporte','label' => Yii::$app->globals->getTraductor('1', Yii::$app->session['idiomaId'], 'folioReporte'),'value' => 'folioReporte','format'=> 'raw'],
							...
							'options' => ['style' => 'width:50px;'],
									...
									'buttons' => [
										'update' => function ($url, $model) {
											$url_group = 'index.php?r=reportespdf/update&f='.$_GET['f'].'&id='.$model->reportesPdfID;
											return Html::a(
    ...
    function confirmDelete(id){
        alertify.confirm('<?= Yii::$app->globals->getTraductor(17, Yii::$app->session['idiomaId'], 'Confirmación') ?>', '¿<?= Yii::$app->globals->getTraductor(9, Yii::$app->session['idiomaId'], 'Seguro que desea eliminar el registro') ?>?', 
            function(){
                $.ajax({
                     type: 'POST',
                     url: "index.php?r=reportespdf/delete",
                     data:{id:id},
