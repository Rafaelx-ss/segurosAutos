...
$ErrorDetenerCiclo .= "usuarioApiID : ".Yii::$app->session['USUARIOSAPILISTA'];
...
try{
	...
	if (file_exists($incluir)) {
		...
		try{
			...
			$validaExistenciaApiLocal=false;
			$RegistrosErrores .= "obtener datos : ".$txtBanderaApi.": ".$listadoApi->ObtenerDatos($txtBanderaApi)."<br>";
		}
		...
		if($validaExistenciaApiLocal){	
			...
			$mensajeGeterror .= "<br /><b>Direccion Descarga:</b> ".$servidor.$RutaApi;
			...
			$mensajeGeterror .= "<br /><b>Identificador:</b> ";
			$mensajeGeterror .= Yii::$app->session['IDENTIFICADOR'];
			...
			$aplicacionID= Yii::$app->session['APLICACION_ID'];
				...
				if(isset($arraydatos['mensaje'])){
					 $mensajeGeterror .= "<br>Mensaje del api de conexion : <strong>".$arraydatos['mensaje']."</strong><br>";
				}
				...
				if(isset($arraydatos['resultado']) or Yii::$app->session['TIPOMETODO']=="POST"){
					try{
						try{
                            ...
                            $mensajeGeterror .= "<br /><b>Api no encontrada, revisar archivos fisicos y configuraciones:</b>". $ex->getMessage()."<br>";
                        }
						...
						if($coleccion != "" or Yii::$app->session['TIPOMETODO']=="POST"){
							...
							if(count($coleccion) > 0){
								...
								if(Yii::$app->session['TIPOMETODO']=="POST"){ 
									try{
                                                                                //echo "<br /><b>Inicio ObtenerDatos</b> ";
										$dato1=0;
										...
										if(count($datosInsertados2)>0){
											...
											$mensajeGeterror .= "Total de filas consultadas: ".count($datosInsertados2)."<br />";
											foreach($datosInsertados2 as $item4){
												...
												if($arraydatosReturn['mensaje']<>""){
													...
													$RegistrosErrores .= "<BR />".$arraydatosReturn['mensaje']."<br>";
												}
												if($arraydatosReturn['resultado'] and strtoupper($arraydatosReturn['mensaje'])=='DATOS CREADOS'){
													...
													$RegistrosErrores .= "<BR />Fila Error: ". $contadorReturn ." ". $arraydatosReturn['mensaje']."<br />".$parametrosR."<br />";
												}
												//echo $parametrosR;
												$contadorReturn++;
												if(trim($arraydatosReturn['mensaje'])=="Acceso a metodo no permitido"){
													
													 $ErrorDetenerCiclo .="<br />Se detuvo el ciclo por que no se cuenta con permisos para el meétodo solicitado";
													break;
												}
												if(trim($arraydatosReturn['mensaje'])=="método POST no implementado"){	
													//aqui hay un echo $ErrorDetenerCiclo .=
													$ErrorDetenerCiclo .="<br />Se detuvo el ciclo por que el método POST no esta Implementado para el API seleccionado";			
											...
											$RegistrosErrores .= "La consulta local no trajo resultados ".$migracion."<br>";
										}
										if($validaEjecucion){
											$colorEvento = "verde";
											...
											$RegistrosErrores .= $mensaje2."<br>";
										}else{
											$colorEvento = "rojo";
											echo "La ejecución del api no se realizó correctamente";
											$mensajeCorto = "La ejecución del api no se realizó correctamente<br>";
											$RegistrosErrores .= "La ejecución del api no se realizó correctamente<br>";
										}
										
									}catch (Exception $e){
										$colorEvento = "rojo";
										echo "Error al consultar datos locales, SIN registros para enviar a método POST: ".$e->getMessage();
										$mensajeCorto = "Error al consultar datos locales, SIN registros  para enviar a método POST<br>";
										$RegistrosErrores .= "Error al consultar datos locales, SIN registros  para enviar a método POST: <br />".$e->getMessage()."<br>";
									...
									if($validaEjecucion){
										...
										try{
												...
												//foreach($coleccion as $item3){
													...
													if($item3['estadoEjecucion']){
														...
														if($contadorErrores2 == 0){
															$mensajeGeterror .= "<br /><b>Script con errores:</b>";
														}
														$mensajeGeterror .= "<br />";
														$mensajeGeterror .= "Script ID: <b>".$item3['scriptID']."</b> Detalle script ID: <b>".$item3['detalleScriptID']."</b><br>
											...
											if($validaEjecucion){
												$colorEvento = "verde";
												 $mensaje2 .= "</br ><b>Se ejecutaron " . $contadorExitos2 . " registros de " . $contador."</b><br />Registros erroneos: " . $contadorErrores2;
												$RegistrosErrores .= "Ejcutados: ".$mensaje2."<br><br>Errores:".$mensajeGeterror."<br>";
												$mensajeCorto = "Ejcutados: ".$mensaje2."<br><br>Errores:".$mensajeGeterror."<br>";
											}else{
												//echo "La ejecución del api no se realizó correctamente";
												$RegistrosErrores .= "Error: La ejecución no se realizó correctamente".$mensajeGeterror."<br>";
											...
											$RegistrosErrores .= "Error :".$e->getMessage()."<br>";
										}
									}else{
										$colorEvento = "rojo";
										echo "No se pudo ejecutar el script del api";
										$RegistrosErrores .= "No se pudo ejecutar el script del api<br>";
									...
									try{										                           
										...
										$mensajeGeterror .= "<br>Error al insertar los datos localmente, ".$e->getMessage()."<br>";//.'<br /> <br />Consulta: <br />';
										$RegistrosErrores .= "Error : ".$e->getMessage()."<br>";
								...
								try{
									if(trim($objeto) <> "EjecucionScripts" and Yii::$app->session['TIPOMETODO']<>"POST"){//$aplicacionID <> "1" AND 
										if($validaEjecucion){
											$colorEvento = "verde";
											 $mensaje2 .= "<b>Se guardaron " . count($coleccion) . " registros de ".count($coleccion)."</b>";
											$exito=true;
											$RegistrosErrores .= $mensajeGeterror.$mensaje2."<br>";
											...
											$RegistrosErrores .= "<br>Error: ".$mensajeGeterror."<br> no se pudo validar la ejecucion del api<br>";
										}
									}
								}
								catch (Exception $e){
									...
									$RegistrosErrores .= "Ocurrio un error en la ejecucion del script ".$e->getMessage()."<br>".$mensajeGeterror."<br>";
								}
								
							}else{
								$colorEvento = "naranja";
								$mensajeCorto = "Alerta : No hay registros para actualizar <br>";
								$RegistrosErrores = "Alerta : No hay registros para actualizar <br>".$mensajeGeterror."<br>";
							}
						}else{
							$exito=true;
							 $colorEvento = "naranja";
							$mensajeCorto = "La tabla de origen: ".trim($txtBanderaApi)." no contiene datos para actualizar o hay un error en la conexion";
							 $mensaje2 .= "<br /><b>La tabla de origen: ".trim($txtBanderaApi)." no contiene datos para actualizar o hay un error en la conexion</b><br>";
						...
						$RegistrosErrores .= "Error de datos:".$e->getMessage()."<br>";
					...
					if(isset($arraydatos['Mensaje'])){
						...
						$RegistrosErrores .= "Error de conexion con el api: ".$arraydatos['mensaje']."<br>";
					}else{
						$colorEvento = "rojo";
						$mensajeCorto = "Error de conexion con el api";
						$RegistrosErrores .= "Error de conexion con el api: El api no produjo resultados.<br> ".$mensajeGeterror."<br>";
					}
				}
			...
			$mensaje2 .= "No hay datos en " . $txtBanderaApi."<br>";			
			$RegistrosErrores .= $mensaje2."<br>";
		}
		...
		$RegistrosErrores .= "El fichero Modelo" . trim($txtBanderaApi) . ".php no existe<br>";
   ...
   //echo $this->mensaje = $e->getMessage();//.'<br /> <br />Consulta: <br />'.$consulta;
	$colorEvento = "rojo";
	$RegistrosErrores .= "Exception:".$e->getMessage()."<br>";
...
Yii::$app->session->set('RESULTADO_BOTON', $mensajeCorto);
 ...
 if(empty($session['logMigracion'])) {
      $session['logMigracion'] = array($banderaName => array('color'=>$colorEvento, "error"=>$RegistrosErrores, 'mensaje2'=>$RegistrosErrores.$ErrorDetenerCiclo, 'mensajeCorto'=>$mensajeCorto));
 }else{
      $session['logMigracion'] = array_merge(Yii::$app->session['logMigracion'], array($banderaName => array('color'=>$colorEvento, "error"=> $mensaje2, 'mensaje2'=>$RegistrosErrores.$ErrorDetenerCiclo, 'mensajeCorto'=>$mensajeCorto)));
 }
