...
// constructor
	...
	function ObtenerDatos($id=0, $tipoconsulta="", $aplicacionID=0){
		...
		try{
    	$query = "select * from " . $this->NombreTabla . " where 1 " . ($aplicacionID > 0 ? " and apiID in(select apiID from Apis where aplicacionID in(2,3))" : "") . " " . ($id > 0 ? " and metodoApiID = ?" : "");
    	...
    	if($num>0){
			...
			echo "<br /><b>Api no encontrada, revisar archivos fisicos y configuraciones:</b>". $ex->getMessage()."<br>";
       }
