...
// constructor
	...
	function ObtenerDatos( $id=0, $tipoconsulta=""){
    	...
    	$query = "select * from " . $this->NombreTabla . " WHERE 1 " . ($id > 0 ? " and menuID = ?" : "");
...
function Inserta($registros){
		...
		foreach($registros as $item)
		 {
			//echo $item['textoID']."lolol";
			 if($this->ObtenerDatos($item['menuID'], "")){
				...
				$consulta= $consulta . $comaText . "('" . $item["menuID"] . "','" . utf8_encode($item["nombreMenu"]) . "','" . $item["urlPagina"] . "','" . $item["imagen"] . "','" . $item["menuPadre"] . "','" . $item["orden"] . "','" . $item["textoID"] . "','" . $item["versionRegistro"] . "','" . $item["regEstado"] . "','" . $item["regFechaUltimaModificacion"] . "','" . $item["regUsuarioUltimaModificacion"] . "','" . $item["regFormularioUltimaModificacion"] . "','" . $item["regVersionUltimaModificacion"] . "')";
			}
   ...
   try{
        ...
        echo $this->Mensaje2 = "INSERTANDO".$e->getMessage().$consulta;
    }
...
function InsertaRegreso($item){
   ...
   try{
        ...
        echo $this->Mensaje2 = "ACTUALIZANDO".$e->getMessage().$consulta;
    }
