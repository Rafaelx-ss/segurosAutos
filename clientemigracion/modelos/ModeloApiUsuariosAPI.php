...
// constructor
	...
	function ObtenerDatos($id=0){
    	...
    	$query = "select * from " . $this->NombreTabla . " " . ($id > 0 ? " where usuarioApiID = ?" : "");
 //echo $id;
    	// prepare the query
		...
		foreach($registros as $item)
		 {
			//echo $item['textoID']."lolol";
			 if($this->ObtenerDatos($item['usuarioApiID'])){
				...
				$consulta= $consulta . $comaText . "(" . $item["usuarioApiID"] . ",'" . ($item["nombreUsuario"]) . "','" . ($item["passw"]) . "','" . ($item["usuario"]) . "','" . ($item["activoUsuario"]) . "','" . ($item["correoUsuario"]) . "','" . ($item["codigoRecuperacionPassw"]) . "'," .
				$CAMPOfECHA . "," . $item["intentosValidos"] . ",'" . ($item["tiempoCaducidadToken"]) . "','" . ($item["usarSeguridadIP"]) . "','" . ($item["usarSeguridadMac"]) . "','" . ($item["usarLectura"]) . "','" . ($item["usarEscritura"]) . "'," . $item["versionRegistro"] . ",'" . ($item["regEstado"]) . "','" . ($item["regFechaUltimaModificacion"]) . "'," . $item["regUsuarioUltimaModificacion"] . "," . $item["regFormularioUltimaModificacion"] . "," . $item["regVersionUltimaModificacion"] . ")";
			}
