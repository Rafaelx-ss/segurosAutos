...
// constructor
	...
	function ObtenerDatos($id=0, $tipoconsulta=""){
		...
		ELSE{
			$query = "select * from " . $this->NombreTabla . " where templateReporteID in(select templateReporteID from ReportesConfiguraciones where regEstado=1 and reporteConfiguracionID in(select reporteConfiguracionID from MigracionReportes where aplicacionID=2))" . ($id > 0 ? " and templateReporteID = ?" : "");
		}
		echo $query = "select * from " . $this->NombreTabla .  " where 1 " . ($id > 0 ? " and templateReporteID = ?" : "");
